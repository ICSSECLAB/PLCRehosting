...
#define s7200_send 0xb1b4
typedef void (*FuncType)(unsigned char*);
...
int UART_protocol_send_single_fast(const char *s, unsigned int size) {
    // unsigned int crc_checksum = size+1;
    WATCHDOG_EXCITE;
    char c;
    volatile uint32_t rh;
...
int UART_protocol_send_single_200(const char *s, unsigned int size) {
    unsigned char* send_buf = (unsigned char*)0x47000;
    *send_buf = (uint8_t)size+1;
    memcpy(send_buf+1, s, size);
    WATCHDOG_EXCITE;

    // *(uint32_t*)0xfffbb120 = 0x967ea5c3;
    // uint8_t len = *send_buf;
    // uint8_t checksum = 0;
    // if(len > 2){
    ...
    //         checksum += send_buf[i];
    //     }
    //     send_buf[len] = checksum;

    //     uint8_t send_len = len+1;
    //     *(uint32_t*)0xfffbc028 |= 0x4000000u;
    //     for(int i = 0; i < 50000; i++){
    //         __asm__("nop");
    //     }
    ...
    //     uint8_t v6 = 0;
    //     while(send_len){
    //         while((*(uint32_t*)0xfffb8018 & 0x20) != 0){
    //             __asm__("nop");
    //         }
    //         *(uint32_t*)0xfffb8000 = send_buf[v6++];
    //         send_len--;
    //     }

    //     while((*(uint32_t*)0xfffb8018 & 8) != 0){
    //         __asm__("nop");
    //     }
    ...
    FuncType func = (FuncType)s7200_send;
    func(send_buf);
    return 0;
}
