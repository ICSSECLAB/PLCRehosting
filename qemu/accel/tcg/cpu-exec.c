...
#endif
        // PORTAL: address interrupts here
        if (((cpu->ARM_CPSR >> 7) & 1) == 0 && cpu->IRQ_count > 0){
            CPUClass *cc = CPU_GET_CLASS(cpu);
            cpu->exception_index = 5; // raise IRQ Exception 
            qemu_mutex_lock_iothread();
            cc->tcg_ops->do_interrupt(cpu);
            qemu_mutex_unlock_iothread();
            cpu->exception_index = -1; // reset the exception index to normal
            cpu->IRQ_count--;
        }
        return false;
    }
