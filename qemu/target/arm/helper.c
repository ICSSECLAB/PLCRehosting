...
uint32_t cpsie_ask_counter = 0;
uint32_t int1_insert_counter = 0;
                ...
                CPSRWriteType write_type)
{
    ...
    uint32_t cpsr_before = cpsr_read(env);
    uint32_t before_disable = 0;
    if(((cpsr_before >> 7) & 1) == 1){
        before_disable = 1;
    }
    ...
    if (write_type != CPSRWriteRaw &&
        ...
        if ((env->uncached_cpsr & CPSR_M) == ARM_CPU_MODE_USR) {
            ...
            CPUState *cpu = env_cpu(env);
            const char *irq_str = "irq";
            if ((write_type == CPSRWriteExceptionReturn) && (strcmp(irq_str, aarch32_mode_name(env->uncached_cpsr)) == 0) && (cpu->in_thread_switch == 1) && (get_ISR_num(cpu->ISR_addr) == 31)){
                // signify the device to exit this irq
                qemu_log_mask(LOG_GUEST_ERROR, "Manual quit thread switch interrupt\n");
                // uint32_t res = tcp_send_to_agent(CMD_EXIT_IRQ, CMD_NOP, CMD_NOP, CMD_NOP);
                // if (res != 0){ // has more IRQs
                //     cpu->IRQ_count = 2;
                //     // set ISR addr
                //     cpu->ISR_addr = res;
                // }
                cpu->in_thread_switch = 0;
            }
    ...
    // CPUState *cpu = env_cpu(env);
    cpu->ARM_CPSR = cpsr_read(env);

    // when write to cpsr and interrupt opened, we ask for interrupts

    if (((cpu->ARM_CPSR >> 7) & 1) == 0 && before_disable){ // current interrupt is open
        if(cpu->IRQ_count == 2){ // already a pending continuing irq
            cpu->IRQ_count = 1;
            qemu_log_mask(LOG_GUEST_ERROR, "[+] take continuing IRQ\n");
            cpu_loop_exit(cpu);
        }else{
            ...
            if(cpu->force_ask == 1){
                ...
                qemu_log_mask(LOG_GUEST_ERROR, "Manual trigger thread switch interrupt: force_ask consumed\n");
                cpu->force_ask = 0;
                cpu_loop_exit(cpu);
                return;
            }
            // ask device for IRQ if ask_irq opened
            if(cpsie_ask_counter++ % 20 == 0){
                qemu_log_mask(LOG_GUEST_ERROR, "[+] cpsie Asking for IRQs\n");
                if (cpu->in_thread_switch == 1 && (int1_insert_counter++ % cpu->int1_freq == 0)){
                    cpu->IRQ_count = 1; 
                    cpu->ISR_addr = get_ISR_addr(1);
                    qemu_log_mask(LOG_GUEST_ERROR, "Manual insert interrupt 0x%x\n",cpu->ISR_addr);
                    cpu_loop_exit(cpu);
                    return;
                }
                // if(cpu->force_ask == 1){
                //     cpu->force_ask = 0;
                // }
                // signify the device to enable interrupts
                // ask if there are irqs to take
                uint32_t res = tcp_send_to_agent(CMD_ASK_IRQ, CMD_NOP, CMD_NOP, CMD_NOP);

                if(res != 0){ // there is a pending IRQ
                    // set ISR address
                    cpu->IRQ_count = 1;
                    cpu->ISR_addr = res;
                                   ...
                                   uint32_t newpc)
{
    ...
    env->regs[15] = newpc;
    arm_rebuild_hflags(env);
    qemu_log_mask(LOG_GUEST_ERROR, "[+] Enter IRQ update ARM_CPSR: 0x%x\n", cpsr_read(env));
    CPUState *cs = env_cpu(env);
    cs->ARM_CPSR = cpsr_read(env);
}
